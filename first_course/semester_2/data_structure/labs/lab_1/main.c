...
int main(int argc, char* argv[]) {
    if (argc != 5) {
        fprintf(stderr, "Usage: %s <input_file> <replace_words_file> <replacement_words_file> <output_file>\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    if (!input_file || !words_file || !replace_file || !output_file) {
    ...
    if (replace_count != replacement_count) {
        fprintf(stderr, "Error: Mismatched replace and replacement word counts\n");
        return EXIT_FAILURE;
    }
